...
public class VRGazeController : MonoBehaviour {
    ...
    #region Raunak's raycast method
    ...
    {
        // Show the debug ray if ticked in inspector
        if (_showDebugRay)
        {
            Debug.DrawRay(transform.position, transform.forward * _debugRayLength, Color.blue, _debugRayDuration);
        }
        ...
        if (Physics.Raycast(transform.position, transform.forward, out RaycastHit hitInfo, 1000f, interactablesLayer))
        {
            _target = hitInfo.transform.GetComponent<VRTargetItem>();
            if (_target != null)
            {
            ...
            else //Raycast didn't hit a valid target
                _target = null;
        }
        ...
        _eventSystem.RaycastAll(m_pointerEventData, results);       //set raycast all to allow both UI and meshes to be interactable, casts rays of all supported raycast types

        
        //loop through all items hit and check for VRTargetItem script
        for (int i = 0; i < results.Count; i++)
        {
            ...
            if (_target != null)
            {
                ...
                debugCube.position = _target.transform.position;
